...
class VacuumPlanning(Problem):
    ...
    def path_cost(self, curNode, state1, action, state2):
        ...
        if (self.env.costFunc == costFunctions[0]):  
            ...
            turn_cost = self.computeTurnCost(curNode.action, action)# Calculate turn cost
            cost += turn_cost + 1 
        elif (self.env.costFunc == costFunctions[2]):  # cost function to encourage staying left of grid
            cost += 1  # Basic move
            if state2[0] >= self.env.width // 2:  # If moving to the right half
                cost += 2
        else:  
            cost += 1  
            if state2[1] >= self.env.height // 2:  # If moving to bottom half
                cost += 2 
        return cost
    ...
    def findMinManhattanDist(self, pos):
        ...
        dirty_rooms = self.env.dirtyRooms
        print("dirty rooms ", dirty_rooms)
        if not dirty_rooms:  # If no dirty rooms left
         return 0
    
        return min(distance_manhattan(pos, dirt) for dirt in dirty_rooms)
    ...
    def findMinEuclidDist(self, pos):
        ...
        dirty_rooms = self.env.dirtyRooms  # Get all dirty room locations
        if not dirty_rooms:  # If no dirty rooms 
            return 0
        
        return min((dirt[0] - pos[0])**2 + (dirt[1] - pos[1])**2 for dirt in dirty_rooms)
...
def breadth_first_graph_search(problem):
    ...
    while frontier:
        ...
        explored.add(tuple(node1.state))
        for  child in node1.expand(problem):
            s = child.state
...
def depth_first_graph_search(problem):
    ...
    if problem.goal_test(node.state):
        return node, set()
    frontier = [Node(problem.initial)]  # Frontier works as STACK
     
    explored = set()

    while frontier:
        node1 = frontier.pop() #LIFO popping from stack
        if problem.goal_test(node1.state):
            return node1, explored
        explored.add(tuple(node1.state))
        for  child in node1.expand(problem):
    ...
    while frontier:
        ...
        for child in node.expand(problem):
            if tuple(child.state) not in explored and child not in frontier:
                ...
                if f(child) < frontier.__getitem__(child):
